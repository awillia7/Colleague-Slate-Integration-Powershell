...
function Get-ColleagueApplicationData {
    ...
    , STPR_STATUS_DATE
    , RANK() OVER (PARTITION BY LEFT(STUDENT_PROGRAMS_ID, 7) ORDER BY STPR_STATUS, STPR_STATUS_DATE DESC, RIGHT(STUDENT_PROGRAMS_ID, LEN(STUDENT_PROGRAMS_ID) - 8)) AS [RANK]
    FROM STPR_STATUSES
    INNER JOIN APP_REC_ORGS AS aro ON aro.APPLICANTS_ID = LEFT(STUDENT_PROGRAMS_ID, 7)
        AND APP_REC_ORG_IDS = 'SLATE'
    WHERE STPR_STATUSES.POS = 1
    AND STPR_STATUSES.STPR_STATUS IN ('A', 'C')
	AND RIGHT(STUDENT_PROGRAMS_ID, 11) <> 'PSEO.TRD.ND'
    )
